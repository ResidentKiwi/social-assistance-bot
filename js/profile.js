...
export default async function () {
  ...
  const avatarHtml = profile.has_avatar
    ? `<img src="/profile/avatar" alt="Avatar" class="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-2 border-violet-500 shadow-md">`
    ...
    <div class="max-w-lg mx-auto bg-gray-900 text-white p-6 mt-8 rounded-xl shadow-lg">
      ...
      <form id="profileForm" class="space-y-4">
        <input type="text" id="name" placeholder="Nome completo" value="${profile.name || ''}"
          required class="w-full p-3 rounded bg-gray-800 border border-gray-700 focus:border-violet-500 focus:outline-none" />
        ...
        <textarea id="desc" placeholder="Sobre você (opcional)" rows="4"
          class="w-full p-3 rounded bg-gray-800 border border-gray-700 focus:border-violet-500 focus:outline-none">${profile.description || ""}</textarea>
        <button type="submit"
